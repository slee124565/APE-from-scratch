...
class APD:
    def __init__(self, num_prompts, starting_prompt, df_train, metaprompt_template_path, generation_model_name,
        ...
        self.generation_model = openai.AsyncOpenAI(api_key=config.get_openai_api_key())
    ...
    async def generate_with_backoff(self, metaprompt):
        ...
        # )
        # return response
        response = await self.generation_model.chat.completions.create(
            messages=[
...
if __name__ == "__main__":
    ...
    generation_model_name = config.get_opro_model_name()
    generation_config = {
        "temperature": 0.7,
    }
    safety_settings = {
